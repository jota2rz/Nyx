...
void UProxyBackendNetDriver::OnIrisRootObjectPostInit(UE::Net::FNetRefHandle RootHandle, UObject* RootObject, UE::Net::FNetObjectFactoryId FactoryId)
{
	...
	UE_LOG(LogNetProxy, Verbose, TEXT("[%s] Attaching root object %s (%s) to %s"), *GetNameSafe(this), *GetNameSafe(RootObject), *RootHandle.ToString(), *GetNameSafe(ProxyNetDriver));
	FrontendBridge->StartReplicatingRootObject(RootObject, UObjectReplicationBridge::FRootObjectReplicationParams(), FactoryId);
}
...
void UProxyBackendNetDriver::OnIrisRootObjectDetached(UE::Net::FNetRefHandle RootHandle, UObject* RootObject, UE::Net::EDetachReason Reason)
{
	...
	UE_LOG(LogNetProxy, Verbose, TEXT("[%s] Detaching root object %s (%s) from %s"), *GetNameSafe(this), *GetNameSafe(RootObject), *RootHandle.ToString(), *GetNameSafe(ProxyNetDriver));
...
void UProxyBackendNetDriver::OnIrisSubObjectPostInit(UE::Net::FNetRefHandle SubObjectHandle, UObject* SubObject, UE::Net::FNetRefHandle RootObjectHandle, UObject* RootObject, UE::Net::FNetObjectFactoryId FactoryId)
{
	...
	{
		UE_LOG(LogNetProxy, Verbose, TEXT("[%s] Attaching sub-object %s (%s) to %s (%s) on %s"), 
			   *GetNameSafe(this), 
...
void UProxyBackendNetDriver::OnIrisSubObjectDetached(UE::Net::FNetRefHandle SubObjectHandle, UObject* SubObject, UE::Net::FNetRefHandle RootObjectHandle, UObject* RootObject, UE::Net::EDetachReason Reason)
{
	...
	UE_LOG(LogNetProxy, Verbose, TEXT("[%s] Detaching sub-object %s (%s) from %s (%s) on %s"), 
		   *GetNameSafe(this), 
		   *GetNameSafe(SubObject), *SubObjectHandle.ToString(),
		   *GetNameSafe(RootObject), *RootObjectHandle.ToString(),
		   *GetNameSafe(ProxyNetDriver));
	...
	if (UActorComponent* SubObjectAsActorComponent = Cast<UActorComponent>(SubObject))
	{
		if (SubObjectAsActorComponent->GetOwner() != OwningActor)
		{
			...
			else if (MatchingComponents > 1)
			{
				...
				UE_LOG(LogNetProxy, Verbose, TEXT("RPC: Found an actor %s with more than one component %s."), *OwningActor->GetName(), *SubObject->GetName());
			}
			else
			{
	...
	if (TObjectPtr<const APlayerController>* GamePlayerControllerPtr = PlayerToPrimaryGameServerPlayerController.Find(OwningPlayer))
	{
		...
		if (GameConnection)
		{
			...
			if (ensure(bIsConnectionToGameServer))
			{
				...
				{
					UE_LOG(LogNetProxy, Verbose, TEXT("RPC: Forwarding %s on %s to game server connection %s:%s for player %s"), *GetNameSafe(Function), *GetNameSafe(OwningActor), *GameConnection->Driver->GetName(), *GetNameSafe(GameConnection), *GetNameSafe(OwningPlayer));
...
void UProxyNetDriver::TickFlush(float DeltaSeconds)
{
	...
	if (bIris)
	{
		...
		for (TActorIterator<AActor> It(World); It; ++It)
		{
			...
			if (Actor->GetOwner() == nullptr && !Actor->IsA(APlayerController::StaticClass()))
			{
				continue;
			}
			...
			{
				if (Actor == Pair.Value || Owner == Pair.Value)
				{
	...
	if (UReplicationSystem* DriverReplicationSystem = GetReplicationSystem())
	{
		if (UObjectReplicationBridge* ObjectReplicationBridge = DriverReplicationSystem->GetReplicationBridgeAs<UObjectReplicationBridge>())
		{
			...
			if (RefHandle.IsValid())
			{
				...
				DriverReplicationSystem->SetReplicationConditionConnectionFilter(RefHandle, UE::Net::EReplicationCondition::RoleAutonomous, ConnectionId, true);
			}
		}	
	}
}
