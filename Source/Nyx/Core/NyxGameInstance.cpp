...
void UNyxGameInstance::Init()
{
	...
	// If -ProxyGameServers= or -ProxyRegistrationPort= is on the command line,
	// this process should run as a MultiServer proxy.  Swap the GameNetDriver
	// definition from IpNetDriver to ProxyNetDriver BEFORE UWorld::Listen()
	// creates the NetDriver.
	...
	if (CmdLine.Contains(TEXT("-ProxyGameServers=")) || CmdLine.Contains(TEXT("-ProxyRegistrationPort=")))
	{
		UE_LOG(LogNyx, Log, TEXT("Detected proxy flag — configuring as PROXY server"));
