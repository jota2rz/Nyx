 ...
 *
 * Cross-server migration uses the DSTM framework (UE 5.7):
 *   When a player crosses a zone boundary, TransferObjectOwnershipToRemoteServer()
 *   serializes the PlayerController + Pawn, sends via beacon RPC to the destination
 *   server, which deserializes and binds the connection — zero client disruption.
...
public:
	...
	/** Address of the server to transfer players to when they cross the zone boundary (non-proxy fallback). */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Nyx|Server")
...
private:
	...
	// is active, uses the engine's built-in DSTM framework for seamless
