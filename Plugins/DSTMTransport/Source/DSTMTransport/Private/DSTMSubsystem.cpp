...
DEFINE_LOG_CATEGORY_STATIC(LogDSTMSubsystem, Log, All);
	...
	const TArray<FString>& PeerAddresses)
{
	if (DSTMNode)
	{
		UE_LOG(LogDSTMSubsystem, Warning,
	...
	if (DSTMNode)
	{
		...
		{
			UE_LOG(LogDSTMSubsystem, Log, TEXT("  DSTM Peer: %s"), *Addr);
		}
	}
	else
	{
		UE_LOG(LogDSTMSubsystem, Error, TEXT("Failed to create DSTM mesh"));
	}
}
...
void UDSTMSubsystem::ShutdownMesh()
{
	if (DSTMNode)
	{
		UE_LOG(LogDSTMSubsystem, Log, TEXT("Shutting down DSTM mesh"));
		DSTMNode = nullptr;
		PeerBeacons.Empty();
...
void UDSTMSubsystem::TransferActorToServer(AActor* Actor, FRemoteServerId DestServerId)
{
	if (!Actor)
	{
		UE_LOG(LogDSTMSubsystem, Error, TEXT("TransferActorToServer: Actor is null"));
		return;
	}
